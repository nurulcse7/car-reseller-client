...
const ToyotaBookingModal = ({ carInfo, setCarInfo }) => {
  const { title, sale_price } = carInfo;
  const { user } = useContext(AuthContext);

  const handleBmwBooking = (event) => {
    ...
    fetch('https://car-reseller-server.vercel.app/toyotaBookings', {
      method: 'POST',
      headers: {
      ...
      .then((data) => {
        ...
        if (data.acknowledged) {
          setCarInfo(null);
          toast.success('You have booked a Toyota car');
          // refetch();
        } else {
...
export default ToyotaBookingModal;
